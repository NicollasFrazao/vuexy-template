...
class ResponsivenessTest extends TestCase
{
    ...
    public function bootstrap5_css_is_loaded_for_responsive_styles(): void
    {
        ...
        // Verificar que o CSS é carregado (via Vite manifest, asset hash, ou referência direta)
        $hasBootstrap = str_contains($html, 'bootstrap') ||
                        str_contains($html, 'core.css') ||
                        str_contains($html, 'theme-default.css') ||
                        str_contains($html, 'core-') || // Hash do Vite (ex: core-Abc123.css)
                        str_contains($html, '/build/assets/') || // Assets compilados pelo Vite
                        str_contains($html, 'rel="stylesheet"'); // Qualquer stylesheet

        $this->assertTrue(
            $hasBootstrap,
            'Layout must load Bootstrap CSS for responsive design (via Vite build or direct link)'
        );
    }
